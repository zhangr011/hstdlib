    ...
    Funs = lists:map( 
             fun(Cfg) ->
                     try
                         ...
                         case P of 
                             [] ->
                                 Config_value = hmisc:term_to_string(erlang:apply(hconfig_app, F, [])),
                                 lists:concat([F, '()->', Config_value, End, '\t\n']);
                             [P1] ->
                                 Config_value = hmisc:term_to_string(erlang:apply(hconfig_app, F, [P1])),
...
get_scene_virtual() ->
    Scene_virtual_ids = hconfig_app:get_scene_virtual(),
    Funs = lists:map( 
             fun(Id) ->
                     Num = hconfig_app:get_scene_virtual_number(Id),
...
get_infant_ctrl() ->
    case hconfig_app:get_infant_ctrl() of
        1 ->
